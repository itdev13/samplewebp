...
router.put('/:locationId',
  ...
  asyncHandler(async (req, res) => {
    ...
    if (updates.xenditApiKey) {
      location.xenditApiKey = encryption.encrypt(updates.xenditApiKey);
    }

    if (updates.xenditWebhookToken) {
      location.xenditWebhookToken = encryption.encrypt(updates.xenditWebhookToken);
    }

    // Update other fields
    if (updates.enabledPaymentMethods) {
      location.enabledPaymentMethods = updates.enabledPaymentMethods;
    }

    if (updates.defaultCurrency) {
      location.defaultCurrency = updates.defaultCurrency;
    }
